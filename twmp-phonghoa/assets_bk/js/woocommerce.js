/******/ (() => { // webpackBootstrap
/*!******************************************!*\
  !*** ./twmp-phonghoa/src/woocommerce.js ***!
  \******************************************/
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function ($) {
    ...
    $('body').delegate('.js-add-to-cart', 'click', function (e) {
      ...
      var productId = Number($buttonEl.data('product-id')) > 0 ? $buttonEl.data('product-id') : Number($buttonElValue) > 0 ? $buttonElValue : 0;
      if (productId > 0) {
        e.preventDefault();
        var data = {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.error && response.product_url) {
              ...
              $('body').trigger('added_to_cart', [response.fragments, response.cart_hash, $buttonEl]);
            }
          }
        });
      }
