...
class Breadcrumbs
{
	...
	public function __construct($args = array())
	{
		...
		$this->args = apply_filters('Twmp_Breadcrumb_args', wp_parse_args($args, $defaults));
	...
	public function trail()
	{
		...
		if (0 < $item_count) {
			...
			if (true === $this->args['show_browse']) {
				...
				$breadcrumb .= sprintf(
					'<%1$s class="screen-reader-text">%2$s</%1$s>',
			...
			$breadcrumb .= sprintf(
				'<%s class="breadcrumbs__list" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			foreach ($this->items as $item) {
				...
				if (!empty($matches)) {
					$item = preg_replace('/(<a.*?)([\'"])>/i', '$1$2 itemprop=$2item$2>', $item);
				} elseif ($item_count !== $item_position) {
					$item = sprintf('<span itemprop="item">%s</span>', $item);
				}
				...
				if (1 === $item_position && 1 < $item_count)
					$item_class .= ' breadcrumbs__item--begin';

				elseif ($item_count === $item_position)
					$item_class .= ' breadcrumbs__item--end';
			...
			$breadcrumb = sprintf(
				'<%1$s aria-label="%2$s" class="breadcrumbs__nav" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters('Twmp_Breadcrumb', $breadcrumb, $this->args);
	...
	protected function set_labels()
	{
		...
		$defaults = array(
			'browse' => esc_html__('Browse:', 'twmp-phonghoa'),
			'aria_label' => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'twmp-phonghoa'),
			'home' => esc_html__('Home', 'twmp-phonghoa'),
			'error_404' => esc_html__('404 Not Found', 'twmp-phonghoa'),
			'archives' => esc_html__('Archives', 'twmp-phonghoa'),
			// Translators: %s is the search query.
			'search' => esc_html__('Search Results for: %s', 'twmp-phonghoa'),
			// Translators: %s is the page number.
			'paged' => esc_html__('Page %s', 'twmp-phonghoa'),
			// Translators: %s is the page number.
			'paged_comments' => esc_html__('Comment Page %s', 'twmp-phonghoa'),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute' => esc_html__('Minute %s', 'twmp-phonghoa'),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week' => esc_html__('Week %s', 'twmp-phonghoa'),
		...
		$this->labels = apply_filters('Twmp_Breadcrumb_labels', wp_parse_args($this->args['labels'], $defaults));
	}
	...
	protected function set_post_taxonomy()
	{
		...
		$this->post_taxonomy = apply_filters('Twmp_Breadcrumb_post_taxonomy', wp_parse_args($this->args['post_taxonomy'], $defaults));
	}
	...
	protected function add_items()
	{
		...
		$this->items = array_unique(apply_filters('Twmp_Breadcrumb_items', $this->items, $this->args));
	}
	...
	protected function add_network_home_link()
	{
		...
		if (is_multisite() && !is_main_site() && true === $this->args['network'])
			$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(network_home_url()), $this->labels['home']);
	}
	...
	protected function add_site_home_link()
	{
		...
		$rel = ' itemtype="https://schema.org/WebPage"';
	...
	protected function add_minute_hour_archive_items()
	{
		...
		if (true === $this->args['show_title'])
			$this->items[] = sprintf($this->labels['archive_minute_hour'], get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'twmp-phonghoa')));
	}
	...
	protected function add_minute_archive_items()
	{
		...
		// Add the minute item.
		if (true === $this->args['show_title'])
			$this->items[] = sprintf($this->labels['archive_minute'], get_the_time(esc_html_x('i', 'minute archives time format', 'twmp-phonghoa')));
	}
	...
	protected function add_hour_archive_items()
	{
		...
		// Add the hour item.
		if (true === $this->args['show_title'])
			$this->items[] = sprintf($this->labels['archive_hour'], get_the_time(esc_html_x('g a', 'hour archives time format', 'twmp-phonghoa')));
	}
	...
	protected function add_day_archive_items()
	{
		...
		$year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'twmp-phonghoa')));
		$month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'twmp-phonghoa')));
		$day = sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'twmp-phonghoa')));
	...
	protected function add_week_archive_items()
	{
		...
		$year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'twmp-phonghoa')));
		$week = sprintf($this->labels['archive_week'], get_the_time(esc_html_x('W', 'weekly archives date format', 'twmp-phonghoa')));
	...
	protected function map_rewrite_tags($post_id, $path)
	{
		...
		if (is_array($matches)) {
			...
			foreach ($matches as $match) {
				...
				if ('%year%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'twmp-phonghoa'))));

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ('%monthnum%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'twmp-phonghoa'))));

				// If using the %day% tag, add a link to the daily archive.
				elseif ('%day%' == $tag)
					$this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'twmp-phonghoa'))));
